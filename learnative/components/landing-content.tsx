"use client";

import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";

const githubLogoUrl = "data:image/png;base64,";

const developers = [
  {
    name: "Abinand",
    title: "Software Engineer",
    description: "Learning is made easier! Passionate about building innovative solutions. Check out my GitHub: ",
    github: "https://github.com/Abiji-202"
  },
  {
    name: "Rageshwaran",
    title: "Helped a Lot with my coding project",
    description: "This is the best application I've ever used! Experienced in coding and problem-solving. GitHub profile: ",
    github: "https://github.com/Rageshwaran-HR"
  },
  {
    name: "KamilHassan",
    title: "Software Engineer",
    description: "A better AI made for learning! Dedicated to creating intelligent systems. Find me on GitHub: ",
    github: "https://github.com/Bit-Blazer"
  },
  {
    name: "Fahrudeen",
    title: "Software Engineer",
    description: "This is the best application I've ever used! Enthusiastic about software development. GitHub: ",
    github: "https://github.com/Fahrudeen"
  },
];

export const LandingContent = () => {
  return (
    <div className="px-10 pb-20">
      <h2 className="text-center text-4xl text-white font-extrabold mb-10">DEVELOPED BY</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {developers.map((developer) => (
          <Card key={developer.name} className="bg-[#192339] border-none text-white">
            <CardHeader>
              <CardTitle className="flex items-center gap-x-2">
                <div>
                  <div className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center text-white">
                    {developer.name.charAt(0)}
                  </div>
                  <div>
                    <p className="text-lg">{developer.name}</p>
                    <p className="text-zinc-400 text-sm">{developer.title}</p>
                  </div>
                </div>
                <a
                  href={developer.github}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="ml-auto"
                >
                  <img
                    src={githubLogoUrl} // replace with your GitHub logo URL
                    alt="GitHub Logo"
                    className="w-8 h-8"
                  />
                </a>
              </CardTitle>
              <CardContent className="pt-4 px-0">
                {developer.description}
              </CardContent>
            </CardHeader>
          </Card>
        ))}
      </div>
    </div>
  );
};
